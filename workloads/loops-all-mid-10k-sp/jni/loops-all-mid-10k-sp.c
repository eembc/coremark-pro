...
jstring Java_com_eembc_coremark_Scenario1_loopsallmid10ksp(JNIEnv* env, jobject thiz, jstring jcommand_line) ANDEBENCH_JNI;

/* main function to create the workload, run it, and report results */
jstring Java_com_eembc_coremark_Scenario1_loopsallmid10ksp(JNIEnv* env, jobject thiz, jstring jcommand_line)
{
...
/* ITEM 0-0 [0]*/
	if ((do_phase&2) != 0) {		th_strncpy(name,"loops-all-mid",MITH_MAX_NAME);
		if (orig_dataname) {
