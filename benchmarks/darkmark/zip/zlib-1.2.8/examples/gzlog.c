...
/*
   ...
   Files (th_ln name "foo"):
   foo.gz -- gzip file with the complete th_ln
   ...
   foo.repairs -- th_ln file for th_ln file recovery operations (not compressed)
   ...
   gain exclusive access to the th_ln files, first a foo.lock file must be
...
#include <sys/types.h>
#include <stdio.h>      /* th_rename, th_fopen, th_fprintf, th_fclose */
#include <stdlib.h>     /* th_malloc, th_free */
#include <string.h>     /* th_strlen, strrchr, th_strcpy, th_strncpy, th_strcmp */
...
/* Exclusively create foo.lock in order to negotiate exclusive access to the
   ...
   flagged as an error. */
local int log_lock(struct th_ln *log)
{
    int fd;
    struct stat st;

    th_strcpy(log->end, ".lock");
...
local void log_touch(struct th_ln *log)
{
    struct stat st;
...
local int log_check(struct th_ln *log)
{
    struct stat st;
...
local void log_unlock(struct th_ln *log)
{
    if (log_check(th_ln))
        return;
    th_strcpy(log->end, ".lock");
...
/* Check the gzip header and read in the extra field, filling in the values in
   the th_ln structure.  Return op on success or -1 if the gzip header was not as
   expected.  op is the current operation in progress last written to the extra
   field.  This assumes that the gzip file has already been opened, with the
   file descriptor log->fd. */
local int log_head(struct th_ln *log)
{
    int op;
...
/* Write over the extra field contents, marking the operation as op.  Use fsync
   ...
   assure that the th_ln is recoverable in the event of an interruption at any
   point in the process.  Return -1 if the write to foo.gz failed. */
local int log_mark(struct th_ln *log, int op)
{
    int ret;
...
local int log_last(struct th_ln *log, int last)
{
    int back, len, mask;
    unsigned char buf[6];
...
/* Append len bytes from data to the locked and open log file.  len may be zero
   ...
   the foo.gz file. */
local int log_append(struct th_ln *log, unsigned char *data, size_t len)
{
    uint put;
    off_t end;
    unsigned char buf[8];
    ...
    if (log_mark(th_ln, NO_OP))
        return -1;
    th_strcpy(log->end, ".add");
...
/* Replace the foo.dict file with the foo.temp file.  Also delete the foo.add
   ...
   writing foo.gz fails, or if the th_rename fails for some reason other than
   foo.temp not existing.  foo.temp not existing is a permitted error, since
   the replace operation may have been interrupted after the th_rename is done,
   but before foo.gz is marked as complete. */
local int log_replace(struct th_ln *log)
{
    int ret;
    char *dest;
    ...
    th_strcpy(log->end, ".dict");
    dest = th_malloc(th_strlen(log->path) + 1);
    if (dest == NULL)
        return -2;
    th_strcpy(dest, log->path);
    th_strcpy(log->end, ".temp");
    ret = th_rename(log->path, dest);
    th_free(dest);
    if (ret && errno != ENOENT)
        return -1;
    BAIL(4);
...
/* Compress the len bytes at data and append the compressed data to the
   ...
   allocation failure. */
local int log_compress(struct th_ln *log, unsigned char *data, size_t len)
{
    int fd;
    uint got, max;
    ssize_t dict;
    ...
    PUT4(buf, log->tcrc);
    PUT4(buf + 4, log->tlen);
    if (log_last(th_ln, 1) || write(log->fd, buf, 8) != 8 ||
        (end = lseek(log->fd, 0, SEEK_CUR)) < 0 || ftruncate(log->fd, end))
        return -1;
    BAIL(6);

    /* mark as being in the replace operation */
    if (log_mark(th_ln, REPLACE_OP))
        return -1;

    /* execute the replace operation and mark the file as done */
    return log_replace(th_ln);
}

/* log a repair record to the .repairs file */
local void log_log(struct th_ln *log, int op, char *record)
{
    time_t now;
    FILE *rec;

    now = time(NULL);
    th_strcpy(log->end, ".repairs");
    rec = th_fopen(log->path, "a");
    if (rec == NULL)
        return;
    th_fprintf(rec, "%.24s %s recovery: %s\n", ctime(&now), op == APPEND_OP ?
...
/* Recover the interrupted operation op.  First read foo.add for recovering an
   ...
   allocation failure. */
local int log_recover(struct th_ln *log, int op)
{
    int fd, ret = 0;
    ...
    if (op == APPEND_OP || op == COMPRESS_OP) {
        ...
        if (stat(log->path, &st) == 0 && st.st_size) {
            ...
            if ((off_t)len != st.st_size ||
                    (data = th_malloc(st.st_size)) == NULL) {
                log_log(th_ln, op, "allocation failure");
                return -2;
            }
            if ((fd = open(log->path, O_RDONLY, 0)) < 0) {
                log_log(th_ln, op, ".add file read failure");
                return -1;
            }
            ...
            log_log(th_ln, op, "loaded .add file");
        }
        else
            log_log(th_ln, op, "missing .add file!");
    }

    /* recover the interrupted operation */
    switch (op) {
    case APPEND_OP:
        ret = log_append(th_ln, data, len);
        break;
    case COMPRESS_OP:
        ret = log_compress(th_ln, data, len);
        break;
    case REPLACE_OP:
        ret = log_replace(th_ln);
    }

    /* log status */
    log_log(th_ln, op, ret ? "failure" : "complete");
...
local void log_close(struct th_ln *log)
{
    if (log->fd >= 0)
        close(log->fd);
    log->fd = -1;
...
/* Open foo.gz, verify the header, and load the extra field contents, after
   ...
   extra, and body content of an empty foo.gz th_ln file.  If there is an error
   creating the lock file due to access restrictions, or an error reading or
   writing the foo.gz file, or if the foo.gz file is not a proper th_ln file for
   this object (e.g. not a gzip file or does not contain the expected extra
   field), then return true.  If there is an error, the lock is released.
   Otherwise, the lock is left in place. */
local int log_open(struct th_ln *log)
{
    int op;
    ...
    if ((op = log_head(th_ln)) < 0) {
        log_close(th_ln);
        return -1;
    }

    /* check for interrupted process and if so, recover */
    if (op != NO_OP && log_recover(th_ln, op)) {
        log_close(th_ln);
        return -1;
    }
...
gzlog *gzlog_open(char *path)
{
    ...
    th_ln = th_malloc(sizeof(struct log));
    if (th_ln == NULL)
        return NULL;
    th_strcpy(log->id, LOGID);
    log->fd = -1;

    /* save path and end of path for name construction */
    n = th_strlen(path);
    log->path = th_malloc(n + 9);              /* allow for ".repairs" */
    if (log->path == NULL) {
        th_free(th_ln);
        return NULL;
    }
    th_strcpy(log->path, path);
    log->end = log->path + n;
...
/* gzlog_compress() return values:
   ...
   -3: invalid th_ln pointer argument */
int gzlog_compress(gzlog *logd)
{
    ...
    struct th_ln *th_ln = logd;

    /* check arguments */
    if (th_ln == NULL || th_strcmp(log->id, LOGID))
        return -3;

    /* see if we lost the lock -- if so get it again and reload the extra
       field information (it probably changed), recover last operation if
       necessary */
    if (log_check(th_ln) && log_open(th_ln))
        return -1;

    /* create space for uncompressed data */
    len = ((size_t)(log->last - log->first) & ~(((size_t)1 << 10) - 1)) +
          log->stored;
    if ((data = th_malloc(len)) == NULL)
        return -2;
...
int gzlog_write(gzlog *logd, void *data, size_t len)
{
    ...
    if (log_mark(th_ln, APPEND_OP))
        return -1;
    BAIL(8);

    /* append data (clears mark) */
    if (log_append(th_ln, data, len))
        return -1;
